...
-- ── Equipment ──────────────────────────────────────────────────────
-- Room IDs aren't hardcoded here — instead each row looks up the right
-- room by name for that site, falling back to any room that site still
-- has, and finally to no room at all (nullable) if the site currently
-- has none. This way the script still works even if rooms were
-- renamed, recreated, or deleted since the original setup.
...
values
  ('b90-eq1', 'b90',
    coalesce(
      (select id from rooms where site_id = 'b90' and name = 'Power & Gen Yard' limit 1),
      (select id from rooms where site_id = 'b90' limit 1)
    ),
    'B90-R2-EQ001', 'cummins 1.1mva_gen_1', 'Generator', 'Online', 2019, 'Cummins', 'C1100D5', 'CMS-1100-88214', '5901234123457', 'Equipment', 'Facilities Power', 'Datacenter Ops', 'generator 1' || chr(10) || 'cummins 1.1mva' || chr(10) || 'at left', 'B90, Smart Village'),
  ('b90-eq2', 'b90',
    coalesce(
      (select id from rooms where site_id = 'b90' and name = 'Server Hall A' limit 1),
      (select id from rooms where site_id = 'b90' limit 1)
    ),
    'B90-R1-EQ002', 'APC Smart-UPS 40kVA', 'UPS', 'Online', 2021, 'APC', 'SURT40KRMXLI', 'APC-40K-55021', '5901234123464', 'Equipment', 'Facilities Power', 'Datacenter Ops', 'Primary UPS for server hall A, rack row 1', 'B90, Smart Village'),
  ('b90-eq3', 'b90',
    coalesce(
      (select id from rooms where site_id = 'b90' and name = 'Server Hall A' limit 1),
      (select id from rooms where site_id = 'b90' limit 1)
    ),
    'B90-R1-EQ003', 'CRAC Unit 02', 'Cooling', 'Offline', 2018, 'Liebert', 'PDX-30kW', 'LBT-PDX-77310', '5901234123471', 'Equipment', 'Facilities Cooling', 'Datacenter Ops', 'Compressor tripped on high pressure — awaiting parts', 'B90, Smart Village'),
  ('auto-eq1', 'auto',
    coalesce(
      (select id from rooms where site_id = 'auto' and name = 'Power & Gen Yard' limit 1),
      (select id from rooms where site_id = 'auto' limit 1)
    ),
    'AUTO-R2-EQ001', 'cummins 400kva_gen_1', 'Generator', 'Online', 2020, 'Cummins', 'C400D5', 'CMS-400-22190', '5901234123488', 'Equipment', 'Facilities Power', 'Datacenter Ops', 'generator 1 — main yard', 'Auto, Alexandria'),
  ('auto-eq2', 'auto',
    coalesce(
      (select id from rooms where site_id = 'auto' and name = 'Server Hall A' limit 1),
      (select id from rooms where site_id = 'auto' limit 1)
    ),
    'AUTO-R1-EQ002', 'APC Smart-UPS 20kVA', 'UPS', 'Online', 2022, 'APC', 'SURT20KRMXLI', 'APC-20K-90142', '5901234123495', 'Equipment', 'Facilities Power', 'Datacenter Ops', 'Secondary UPS, rack row 1', 'Auto, Alexandria')
...
values
  ...
  ('wo97', 97, 'b90', 'b90-eq2', 'APC Smart-UPS 40kVA', 'Battery health check after alarm', 'Battery health check after alarm', 'High', 'Corrective', 'u3', 'In Progress', null, '2026-08-08', '2026-08-08',
   '["Check UPS event log for the alarm trigger","Test each battery string individually","Record voltage readings","Replace any battery reading below threshold","Take a photo for each step and attach to Files","Sign off and complete work order"]'::jsonb, '1.5h'),
  ('wo98', 98, 'auto', 'auto-eq1', 'cummins 400kva_gen_1', 'Monthly load bank test', 'Monthly load bank test', 'Low', 'Preventive', 'u3', 'Closed', 'MOP-GEN-LoadTest-v1.pdf', '2026-07-15', '2026-07-15',
   '["Connect load bank","Run generator at 50% load for 30 minutes","Record temperatures and pressures","Disconnect load bank","Take a photo for each step and attach to Files","Sign off and complete work order"]'::jsonb, '3.0h')
on conflict (id) do nothing;
